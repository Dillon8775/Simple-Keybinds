...
public class IncreasedBrightnessSliderCallbackBase {
    ...
    public static @NotNull Optional<Double> validateValue(@NotNull Double aDouble) {
        return aDouble >= ModUtil.minBrightness && aDouble <= ModUtil.maxBrightness ? Optional.of(aDouble) : Optional.empty();
    }

    public static double toSliderValue(@NotNull Double aDouble) {
        double range = ModUtil.maxBrightness - ModUtil.minBrightness;
        double offset = ModUtil.minBrightness;
    ...
    public static @NotNull Codec<Double> codec() {
        return Codec.either(Codec.doubleRange(ModUtil.minBrightness, ModUtil.maxBrightness), Codec.BOOL).xmap(either -> either.map(value -> value, value -> value ? 1.0 : 0.0), Either::left);
    }
}